...
setup_branch() {
    ...
    if [ "$target_branch" != "$current_branch" ]; then
        ...
        git remote set-url origin "https://github.com/opensafely-core/job-server.git"
        git fetch --all --prune
...
deploy() {
    ...
    docker exec -t dokku sh -c "dokku git:from-image job-server dokku/job-server:latest" || true
    docker exec -t dokku sh -c "dokku ps:rebuild job-server"
