...
should_deploy() {
    ...
    if ! docker exec dokku dokku apps:exists job-server >/dev/null 2>&1; then
        log "Dokku app 'job-server' does not exist."
        return 0
    fi

    # 4. Dokku app not running
    if ! docker exec dokku dokku ps:report job-server | grep -q running; then
        log "Dokku app 'job-server' is not running."
        return 0
    fi

    # 5. Dokku app not running on 7000
    local port_map
    port_map=$(docker exec dokku dokku ports:report job-server --ports-map 2>/dev/null | grep 7000)
    if [[ -z "$port_map" ]]; then
        log "Dokku app 'job-server' is not running on port 7000."
        return 0
    fi
...
setup_storage() {
  ...
  sudo mkdir -p /var/lib/dokku/data/storage/job-server/

  # Set permissions
  sudo chown -R 10003:10003 /var/lib/dokku/data/storage/job-server
...
deploy_application() {
    ...
    log "Building and tagging job-server application..."
    just docker-build prod
    docker tag job-server dokku/job-server:latest

    log "Creating dokku app..."
    docker exec -t dokku sh -c "dokku apps:destroy job-server --force" || true
    docker exec -t dokku sh -c "dokku apps:create job-server"

    log "Mounting storage for job-server in dokku..."
    docker exec -t dokku sh -c "dokku storage:mount job-server /var/lib/dokku/data/storage/job-server/:/storage"

    log "Setting environment variables in dokku..."
    docker exec -t dokku sh -c "dokku config:set job-server \
        BASE_URL=\"http://localhost:8000\" \
        DATABASE_URL=\"postgres://user:pass@localhost:6542/jobserver\" \
    ...
    docker exec -t dokku sh -c "dokku git:from-image job-server dokku/job-server:latest"
    docker exec -t dokku sh -c "dokku ports:add job-server http:7000:7000"
