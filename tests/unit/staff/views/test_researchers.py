...
from ....factories import ResearcherRegistrationFactory, UserFactory
...
def test_researcheredit_get_success(rf, core_developer):
    ...
    response = ResearcherEdit.as_view()(request, pk=researcher.pk)
...
def test_researcheredit_unknown_researcher(rf, core_developer):
    ...
    with pytest.raises(Http404):
        ResearcherEdit.as_view()(request, pk=0)
...
def test_researcheredit_without_core_dev_role(rf, core_developer):
    ...
    with pytest.raises(PermissionDenied):
        ResearcherEdit.as_view()(request, pk=researcher.pk)
