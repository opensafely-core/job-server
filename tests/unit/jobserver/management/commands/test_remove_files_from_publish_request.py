...
def test_remove_files_from_publish_request():
    out = StringIO()
    err = StringIO()
    snapshot = SnapshotFactory()
    ...
    release_file_to_remove = ReleaseFileFactory(
        workspace=snapshot.workspace, name="/release/test/file_to_remove.csv"
    )
    ...
    assert release_file_to_remove in snapshot.files.all()
